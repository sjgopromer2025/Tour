...
for loc in loc_list:
    ...
    #단어간 상관계수를 구한다. (키워드 네크워크 그래프용)
    noun_total_list = list(set(noun_total_list))
    print(len(noun_total_list))
    print(len(total_word))
    
        
    pos_list = []
    for word in noun_total_list:
        pos_list.append(total_word[word])
    ...
    for val in keyword:
        corrWord = []
        for val2 in noun_total_list:
            corrWord.append(allWord[val].corr(allWord[val2], method="pearson"))
            #print(corrWord)
        data_dict[val] = corrWord    
    ...
    data_dict[0] = noun_total_list
    data_dict[1] = pos_list
    
    
    df = pd.DataFrame(data_dict)
    
    df.to_csv(loc+"corr.csv",encoding="euc-kr")
    break
